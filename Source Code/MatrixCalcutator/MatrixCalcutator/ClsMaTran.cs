...
namespace MatrixCalcutator
{
    public class ClsMaTran
    {
        ...
        public void ReadExcel(string filename)
        {
            ...
            {
                Console.WriteLine("Khong tim thay file " + filename + "!");
                return;
            }
            ...
            try
            {
                this.SoDong = Convert.ToInt32(xlRange[1, 1].Value2);
                this.SoCot = Convert.ToInt32(xlRange[1, 2].Value2);
                for (int i = 0; i < SoDong; i++)
                {
                    for (int j = 0; j < SoCot; j++)
                    {
                        this.Matrix[i, j] = xlRange.Cells[i + 2, j + 1].Value2;
                    }
                }
